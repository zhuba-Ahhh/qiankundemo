...
registerMicroApps(
  [
    ...
    {
      name: "vue-tsx", // 子应用的名称
      entry: "//localhost:4006", // 子应用的访问地址
      container: "#container", // 子应用应该挂载的位置（后面会解释）
      activeRule: "/vue-tsx", // 在路径为xxx的时候让子应用渲染
      loader, // loader是乾坤提供的一个类似于加载中的函数、
      props: {
        routerBase: "/vue-tsx",
      },
  ...
  ],
  {
    ...
    beforeMount: (app: any) => {
      ...
      const appStyleList: Element[] = [];
      Array.from(document.getElementsByTagName("head")[0].children)
        .filter((item) => item.tagName === "STYLE")
        .forEach((element) => {
          const attr = element.getAttribute("data-app");
          if (!attr) {
      ...
      const styleList = styleMap.get(app.name);
      if (styleList) {
        styleList.forEach((s) => {
