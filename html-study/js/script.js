...
function saveTime(status) {
  localStorage.setItem(
    "studyTime",
    JSON.stringify({ start: startTime, pass: passTime, status: status })
  );
}
...
function subTime() {
  PostbirdAlertBox.prompt({
    title:
      '<h4 style="margin-bottom:5px;">时间回溯</h4>若您在计时中做了其他的事情，可以使用此功能进行时间回溯。<br> <span style="color:grey;">如：中途玩了20分钟，输入20并提交计时将会减去20分钟。</span> <br>请输入您想回溯的时间（单位：分钟）',
    okBtn: "确认",
    onConfirm: function (data) {
      ...
      if (
        st.start + Number(data) * 60000 > new Date().getTime() ||
        (st.pass && st.pass < Number(data) * 60000)
      ) {
...
function randomText() {
  document.querySelector(".bottom").innerHTML =
    textls[Math.floor(Math.random() * textls.length)];
}
...
function changeText() {
  PostbirdAlertBox.prompt({
    title:
      '<h4 style="margin-bottom:5px;">更改文本</h4>请输入一段文字替换此区域内容，清除文本则恢复随机显示。',
    okBtn: "确认",
    onConfirm: function (data) {
      if (data) {
...
function changeBg() {
  document.body.style.backgroundImage = "none"; // 先将背景图置空
  const newBackground = "https://bing.icodeq.com"; // 新的背景图URL地址
  document.body.style.backgroundImage = "url(" + newBackground + ")"; // 赋值新的背景图
}
